...
export function getElevatedEdgeZIndex({
  ...
  // Base z-index from edge or default
  let finalZIndex = zIndex;
  
  // If selected and elevate on select is enabled, increase z-index
  if (selected && elevateOnSelect) {
    finalZIndex += 1000; // Elevate selected edges above other elements
  }
  
  // Check if either source or target node has a parent (elevated edge)
  const hasParent = sourceNode.parentId || targetNode.parentId;
  if (hasParent) {
    finalZIndex += 1; // Slight elevation for edges connected to child nodes
  }
