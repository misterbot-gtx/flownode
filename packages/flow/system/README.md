...
```sh
pnpm add @flow/system
```

## What is this package?

`@flow/system` provides core, framework-agnostic helpers and types for building node-based editors and flow diagrams. It contains the logic and utilities that are shared between React Flow and Svelte Flow, such as edge path calculations, pan/zoom, node dragging, and more.
...
```ts
import { getBezierPath, getConnectedEdges, Position, XYPanZoom } from '@flow/system';
```

### Example: Bezier Edge Path Creation

```ts
import { getBezierPath, Position } from '@flow/system';
